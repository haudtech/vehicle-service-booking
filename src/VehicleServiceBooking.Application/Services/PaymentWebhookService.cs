...
{
    private const string WebhookInboxUniqueIndexName = "IX_PaymentWebhookInbox_Provider_Event_Unique";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await _paymentWebhookRepository
            .ExecuteInExecutionStrategyAsync(async ct =>
            {
                await using var transaction = await _paymentWebhookRepository
                ...
                try
                {
                    var existingInbox = await _paymentWebhookRepository
                        .GetInboxByProviderAndEventIdAsync(providerId.Value, normalizedEventId, ct)
                        .ConfigureAwait(false);

                    if (existingInbox != null)
                    {
                        await transaction.CommitAsync(ct).ConfigureAwait(false);
                        return BuildDuplicateResponse(providerType, normalizedEventId, existingInbox);
                    }

                    var receivedStatus = await _paymentWebhookRepository
                        .GetWebhookProcessStatusAsync(PaymentWebhookProcessStatus.Received, ct)
                        .ConfigureAwait(false);
                    ...
                    await _paymentWebhookRepository
                        .AddWebhookInboxAsync(webhookInbox, ct)
                        .ConfigureAwait(false);

                    var paymentOrder = await _paymentWebhookRepository
                        .GetPaymentOrderForWebhookAsync(providerId.Value, request.IntentCode.Trim(), ct)
                        .ConfigureAwait(false);

                    if (paymentOrder == null)
                    {
                        var ignoredStatus = await _paymentWebhookRepository
                            .GetWebhookProcessStatusAsync(PaymentWebhookProcessStatus.Ignored, ct)
                            .ConfigureAwait(false);
                        ...
                        await _paymentWebhookRepository.SaveChangesAsync(ct).ConfigureAwait(false);
                        await transaction.CommitAsync(ct).ConfigureAwait(false);
                    ...
                    if (paymentOrder.PaymentTransaction == null)
                    {
                        var failedStatus = await _paymentWebhookRepository
                            .GetWebhookProcessStatusAsync(PaymentWebhookProcessStatus.Failed, ct)
                            .ConfigureAwait(false);
                    ...
                    var transactionStatusLookup = await _paymentWebhookRepository
                        .GetPaymentTransactionStatusAsync(transactionStatus, ct)
                        .ConfigureAwait(false);
                    ...
                    var intentStatusLookup = await _paymentWebhookRepository
                        .GetPaymentIntentStatusAsync(intentStatusTarget, ct)
                        .ConfigureAwait(false);

                    var orderPaymentStatusLookup = await _paymentWebhookRepository
                        .GetOrderPaymentStatusAsync(orderPaymentStatusTarget, ct)
                        .ConfigureAwait(false);
                    ...
                    var processedStatus = await _paymentWebhookRepository
                        .GetWebhookProcessStatusAsync(PaymentWebhookProcessStatus.Processed, ct)
                        .ConfigureAwait(false);
                ...
                catch (DbUpdateException ex) when (IsWebhookInboxUniqueViolation(ex))
                {
                    await transaction.RollbackAsync(ct).ConfigureAwait(false);
                    _paymentWebhookRepository.ClearChangeTracker();
    ...
    private ProcessPaymentWebhookResponse BuildDuplicateResponse(
        PaymentProviderType providerType,
        string eventId,
        PaymentWebhookInbox existingInbox)
    {
        _logger.LogInformation(
    ...
    private static bool IsWebhookInboxUniqueViolation(DbUpdateException exception)
    {
        var rawMessage = exception.ToString();
        return rawMessage.Contains(WebhookInboxUniqueIndexName, StringComparison.Ordinal);
    }
