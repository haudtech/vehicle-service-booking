...
Architecture-level request/response summary:

- `GET /api/v1/availability`: query-side flow returning available `(TechnicianId, ServiceBayId, slot window)` options.
- `POST /api/v1/appointments`: command-side flow using client-selected resources with two-stage validation:
  - Stage 1: application pre-validation (availability + business rules)
  - Stage 2: PostgreSQL exclusion constraints at commit time
- Idempotency support: optional `Idempotency-Key` for safe retries and replay semantics.
- Failure model: validation errors (`400`) vs booking/idempotency conflicts (`409`) with consistent conflict mapping.

Detailed contract and end-to-end request/response walkthrough moved to:

- `docs/GUIDANCES/API_TEST_GUIDES/REQUEST_RESPONSE_REFERENCE.md`
